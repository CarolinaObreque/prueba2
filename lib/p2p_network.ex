defmodule Prueba2.P2PNetwork do
  ...
  # Eliminar peer por nombre de usuario
  def remove_peer_by_username(username) do
    GenServer.cast(__MODULE__, {:remove_peer_by_username, username})
  end
  ...
  @impl true
  def handle_cast({:remove_peer_by_username, username}, state) do
    # Buscar la dirección correspondiente al nombre de usuario
    address_to_remove = Enum.find_value(state.peers, fn {addr, name} ->
      if name == username, do: addr, else: nil
    end)

    if address_to_remove do
      # Eliminar el peer
      new_peers = Map.delete(state.peers, address_to_remove)
      new_usernames = MapSet.delete(state.usernames, username)

      # Notificar a los peers restantes (usando la función existente broadcast_message)
      broadcast_message("Usuario #{username} ha sido eliminado de la red")
