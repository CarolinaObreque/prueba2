defmodule Prueba2.TeamManager do
  ...
  # Notificar a todos los nodos que un equipo está listo
  def notify_team_ready(team_name) do
    P2PNetwork.get_peers()
    |> Enum.each(fn {address, _} ->
      Task.start(fn ->
        url = "http://#{address}/api/team-ready-update"
        # Add source information to prevent broadcast loops
    ...
    case Map.fetch(state.teams, team_name) do
      {:ok, team} ->
        if MapSet.size(team.players) == 0 do
          ...
          notify_team_ready(team_name) # Notificar a otros nodos

          # Guardar el nuevo estado
          new_state = %{state | teams: new_teams}

          # Verificar si todos los equipos están listos para iniciar el juego automáticamente
          if check_all_teams_ready(new_state) do
            # Programar el inicio automático del juego con un pequeño retraso
            # para dar tiempo a que todos los nodos se sincronicen
            Process.send_after(Prueba2.ShiftManager, :auto_start_game, 500)
          end
  ...
  @impl true
  def handle_call({:set_team_ready_no_broadcast, team_name}, _from, state) do
    case Map.fetch(state.teams, team_name) do
      {:ok, team} ->
        if MapSet.size(team.players) == 0 do
          {:reply, {:error, "El equipo no tiene jugadores"}, state}
  ...
  @impl true
  def handle_call({:update_team_position_sync, team_name, position}, _from, state) do
    # Actualizar posición sin enviar broadcasts para evitar loops infinitos
    case Map.fetch(state.teams, team_name) do
      {:ok, team} ->
        updated_team = %{team | position: position}
        new_teams = Map.put(state.teams, team_name, updated_team)
        {:reply, {:ok, "Posición actualizada"}, %{state | teams: new_teams}}
      :error ->
  ...
  def handle_cast({:update_team_position, team_name, position}, state) do
    case Map.fetch(state.teams, team_name) do
      {:ok, team} ->
        ...
        # Capturar la información necesaria ANTES de pasarla a TeamPositionSync
        # para evitar llamadas recursivas
        teams_info = Map.keys(state.teams)
                    |> Enum.filter(fn t_name -> t_name != team_name end)
                    |> Enum.map(fn t_name ->
                      team_info = Map.get(state.teams, t_name)
                      {t_name, %{
                        players: MapSet.to_list(team_info.players)
                      }}
                    end)
                    |> Enum.into(%{})

        # Pasar los datos del equipo ya procesados para evitar que TeamPositionSync
        # tenga que llamar a TeamManager.get_teams()
        Task.start(fn ->
          Prueba2.TeamPositionSync.broadcast_position_update(team_name, position, teams_info)
        end)
  ...
  # Verifica si todos los equipos están listos para iniciar el juego
  defp check_all_teams_ready(state) do
    teams_with_players = Enum.filter(state.teams, fn {_, team_info} ->
      MapSet.size(team_info.players) > 0
    end)
